...
export default function UpdateEventsForm(params: { token: any, eventsId: any }) {
    ...
    useEffect(() => {
        axios.get(`${process.env.NEXT_PUBLIC_API_URL}/events/${params.eventsId}`, {
            headers: {
            ...
            <Grid templateColumns='repeat(5, 1fr)' gap={6}>
                <GridItem w='100%' colSpan={2} gap={5}>
                    <Box bg={'white'} p={5} rounded={10} shadow={'lg'} mb={10}>
                        ...
                        <Image src={`${process.env.NEXT_PUBLIC_URL}/storage${events.thumbnail}`} alt='Thumbnail'></Image>
                    </Box>
                </GridItem>
                <GridItem w='100%' colSpan={3}>
                    <Box bg={'white'} p={5} rounded={10} shadow={'lg'} mb={10}>
                        <Box mb={5}>
                            ...
                            <Input size='md' name='name' required defaultValue={events.name} />
                        ...
                        <Flex w={'100%'} justifyContent={'space-between'} mb={5}>
                            <Box w={'50%'} mr={3}>
                                ...
                                <Input size='md' name='location_at' required defaultValue={events.location_at} />
                                <Text size={'xs'} fontStyle={'italic'} color={'red'}>{error.data.name ? `* ${error.data.name}` : ''}</Text>
                            </Box>
                            <Box w={'50%'} mr={3}>
                                <Text mb={2}>Link Lokasi</Text>
                                <Input size='md' name='link_location' required defaultValue={events.link_location} />
                                <Text size={'xs'} fontStyle={'italic'} color={'red'}>{error.data.name ? `* ${error.data.name}` : ''}</Text>
                            </Box>
                        </Flex>
                        <Flex w={'100%'} justifyContent={'space-between'} mb={5}>
                            <Box w={'50%'} mr={3}>
                                ...
                                <Input size='md' name='start_at' required type='datetime-local' defaultValue={events.start_at} />
                                <Text size={'xs'} fontStyle={'italic'} color={'red'}>{error.data.name ? `* ${error.data.name}` : ''}</Text>
                            </Box>
                            <Box w={'50%'} mr={3}>
                                <Text mb={2}>Berakhir pada</Text>
                                <Input size='md' name='end_at' required type='datetime-local' defaultValue={events.end_at} />
                                <Text size={'xs'} fontStyle={'italic'} color={'red'}>{error.data.name ? `* ${error.data.name}` : ''}</Text>
                            </Box>
                        </Flex>
                        ...
                        <Box w={'100%'} mr={3}>
                            ...
                            <Editor
                                ...
                                initialValue={events.description}
                                init={{
                                    height: 500,
                                    menubar: false,
                                    plugins: [
