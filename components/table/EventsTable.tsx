...
export default function EventsTable(params: { token: any }) {
    ...
    */
    function handleDeleteData() {
        ...
        axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/events/${deleteUser.id}`, {
            headers: {
                Authorization: `Bearer ${params.token}`
            }
        }).then((response: any) => {
            ...
            {showData.status ? (
                <>
                    <Flex justifyContent={'space-between'} alignItems={'center'} mb={2}>
                        <Heading size={'md'}>{showData.data.name}</Heading>
                    ...
                    <Grid templateColumns='repeat(2, 1fr)' gap={6}>
                        ...
                        <GridItem w='100%'>
                            ...
                            <Text fontSize={'lg'} mb={2}>
                                Lokasi: <Link as={NextLink} href={showData.data.link_location}>{showData.data.location_at}</Link>
                            </Text>
                            <Text fontSize={'lg'} mb={2}>
                                Kegiatan Dimulai: {showData.data.start_at}
                            </Text>
                            <Text fontSize={'lg'} mb={2}>
                                Kegiatan Berakhir: {showData.data.end_at}
                            </Text>
                            <Text fontSize={'lg'} mb={2}>
                                        ...
                                        <Tr key={index}>
                                            ...
                                            <Td>
                                                ...
                                                <Button colorScheme='blue' mx={2} onClick={
                                                    () => router.push(`/dashboard/events/edit/${user.id}`)
                                                }><AiFillEdit /></Button>
