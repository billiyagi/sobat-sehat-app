...
export default function NewsTable(params: { token: any }) {
    ...
    */
    function handleDeleteData() {
        ...
        axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/news/${deleteUser.id}`, {
            headers: {
                Authorization: `Bearer ${params.token}`
            }
        }).then((response: any) => {
            ...
            {showData.status ? (
                <>
                    <Flex justifyContent={'space-between'} alignItems={'center'} mb={2}>
                        <Heading size={'md'}>{showData.data.title}</Heading>
                    ...
                    <Divider mb={3} />
                    <Text fontSize={'lg'} mb={3} fontWeight={'bold'}>Thumbnail</Text>
                    <Grid templateColumns='repeat(2, 1fr)' gap={6}>
                        <GridItem w='100%'>
                            <Box>
                                <Image src={`${process.env.NEXT_PUBLIC_URL}/storage${showData.data.thumbnail}`} alt={`Thumnail post ${showData.data.name}`}></Image>
                            </Box>
                        </GridItem>
                        <GridItem w='100%'>
                            <Text fontSize={'lg'} mb={2}>
                                Author: {showData.data.author.name}
                            </Text>
                            <Text fontSize={'lg'} mb={2}>
                                Kategori: {showData.data.category.name}
                            </Text>
                            <Text fontSize={'lg'} mb={2}>
                                Dibuat pada: {showData.data.created_at}
                            </Text>
                        </GridItem>
                    </Grid>
                    ...
                    <TableContainer maxHeight={'500px'} overflowY={'auto'}>
                        <Table variant='simple'>
                            <Thead bg={'#fba600'}>
                                <Tr >
                                    <Th color={'white'}>#</Th>
                                    <Th color={'white'}>News</Th>
                                    <Th color={'white'}>Author</Th>
                                    <Th color={'white'}>Location At</Th>
                                ...
                                {(dataset[currentPage]) ? (
                                    dataset[currentPage].map((user: any, index: number) => (
                                        <Tr key={index}>
                                            ...
                                            <Td><Button colorScheme='yellow' onClick={() => handleShowData(user)}><FaEye /></Button><Button colorScheme='blue' mx={2} onClick={() => router.push(`/dashboard/news/edit/${user.id}`)}><AiFillEdit /></Button><Button colorScheme='red' onClick={() => {
                                                setDeleteUser({ id: user.id, index: index })
                                                return onOpen();
                                            }}><HiMiniTrash /></Button>
                                            </Td>
                                        </Tr>
